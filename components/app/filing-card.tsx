    ...
    <Card className="group p-4 transition-colors hover:border-primary/30">
      <Link href={`/filings/${filing.id}`} className="block">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto font-mono text-[10px] text-muted-foreground">{filing.type}</span>
        </div>
        <h3 className="mt-2.5 text-sm font-medium text-foreground transition-colors group-hover:text-primary">{filing.title}</h3>
        <div className="mt-3 flex items-center gap-3 text-xs">
          ...
          <span className="font-mono text-muted-foreground">{fmtDate(filing.dueAt, { withYear: true })}</span>
          <span className="text-muted-foreground/40">·</span>
